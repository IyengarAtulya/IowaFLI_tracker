function abc=TrackVideo_v8b(PATH, FILE, numflies, Thresh, roi, chksz,InitCoords,varargin)
...
%         %end fog filter code
        ...
        while OKFlag == 0
            ...
            if (obflies < numflies)
                ...
                zoomframe1 = frames(Ymin1:Ymax1,Xleft1:Xright1,:,j);
                zoomframe2 = frames(Ymin2:Ymax2,Xleft2:Xright2,:,j);
...
%                 zbwframe2 = im2bw(zoomframe2,thresh-(AxisBonus21+AxisBonus22+Divider/100));
            ...
            if currentframe ~= 0
                ...
                if (currentframe > 5 && numel(find(dist < 10))>numflies)
            ...
            %         if currentframe == 1 && stopframe<99999
            %             xyz = figure;
            %         else
            %             figure(xyz)
            %         end
            %         if stopframe-currentframe<10
            %         imshow(rbwframe);
            %         hold on
            %         for k=1:numflies
            %             scatter(output(k*2,currentframe),output(k*2+1,currentframe),20)
            %         end
            %         hold off
            %         end
        end
