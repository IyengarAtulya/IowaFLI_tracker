function varargout = iowaFLI_Tracker_3_0(varargin)
% IOWAFLI_TRACKER_3_0 MATLAB code for iowaFLI_Tracker_3_0.fig
%      IOWAFLI_TRACKER_3_0, by itself, creates a new IOWAFLI_TRACKER_3_0 or raises the existing
%      singleton*.
%
%      H = IOWAFLI_TRACKER_3_0 returns the handle to a new IOWAFLI_TRACKER_3_0 or the
%      handle to
%      the existing singleton*.
%
%      IOWAFLI_TRACKER_3_0('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in IOWAFLI_TRACKER_3_0.M with the given input
%      arguments.
%
%      IOWAFLI_TRACKER_3_0('Property','Value',...) creates a new IOWAFLI_TRACKER_3_0 or
%      raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before iowaFLI_Tracker_3_0_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to iowaFLI_Tracker_3_0_OpeningFcn via varargin.
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @iowaFLI_Tracker_3_0_OpeningFcn, ...
    'gui_OutputFcn',  @iowaFLI_Tracker_3_0_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before iowaFLI_Tracker_3_0 is made visible.
function iowaFLI_Tracker_3_0_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to iowaFLI_Tracker_3_0 (see VARARGIN)

% Choose default command line output for iowaFLI_Tracker_3_0
...
% UIWAIT makes iowaFLI_Tracker_3_0 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = iowaFLI_Tracker_3_0_OutputFcn(hObject, eventdata, handles)
    ...
    for i=1:numel(SelectionValues)
        try
            ...
            if get(handles.Analyze_Choice_chk,'value')
                ...
                Coordinates=TrackVideo_v8b(handles.VideoPaths{i},handles.VideoFiles{i},numflies(i),threshoutput(i).thresh,roiout(i).roi,150,InitCoords,Targets,startFrame);
            else
            ...
            %Coordinates=TrackVideo_NoBackground(handles.VideoPaths{x},handles.VideoFiles{x},numflies(i),threshoutput(i),roiout(i).roi,50,InitCoords);
